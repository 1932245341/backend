...
public class ParkingController {
    ...
    /**
     * 停车场分页
     * @param parkingPageQueryDTO 分页查询参数
     * @return 分页结果
     */
    @GetMapping("/page")
    @Operation(summary = "停车场分页查询", description = "根据条件分页查询停车场数据") // 替换 @ApiOperation
    public Result<PageResult> page(
            @Parameter(description = "分页查询参数") ParkingPageQueryDTO parkingPageQueryDTO // 参数描述
    ) {
        log.info("停车场分页查询：{}", parkingPageQueryDTO);
        PageResult pageResult = parkingService.queryPage(parkingPageQueryDTO);
